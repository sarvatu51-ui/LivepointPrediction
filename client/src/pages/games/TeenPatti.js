...
const TeenPatti = ({ user, onPointsUpdate }) => {
  ...
  const showdown = async () => {
    ...
    setResult({ won, amount: won ? win : stake, m: 1.9 });
    setState('result');
    setLoading(true);
    try {
      const res = await api.post('/casino/play', { game: 'teen_patti', stake, multiplier: won ? 1.9 : 0, result: won ? 'won' : 'lost' });
  ...
  const fold = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/casino/play', { game: 'teen_patti', stake: lose, multiplier: 0, result: 'lost' });
      onPointsUpdate(res.data.newPoints);
    } catch (e) { console.error(e); }
    setLoading(false);
  };
  ...
  const reset = () => {
    setState('idle'); setResult(null); setPlayerCards([]); setDealerCards([]);
    ...
    <div className="tp-game">
        ...
        <div className="tp-center-area">
          ...
          {result && (
            <div className={`tp-result-box ${result.won ? 'won' : 'lost'}`}>
              <div className="tp-res-title">{result.folded ? '🏳️ Folded' : result.won ? '🏆 YOU WIN!' : '😔 DEALER WINS'}</div>
              <div className="tp-res-amt">{result.won ? `+${result.amount} pts` : `-${result.amount} pts`}</div>
            </div>
          )}
        </div>
      ...
      <div className="tp-controls">
          ...
          <div className="tp-qbtns">
            {[10, 50, 100, 500].map(s => (
              <button key={s} className={`tp-qbtn ${stake === s ? 'active' : ''}`}
                onClick={() => setStake(s)} disabled={state === 'playing' || state === 'dealing'}>{s}</button>
            ))}
          </div>
        </div>
        <div className="tp-actions">
          ...
          {state === 'dealing' && (
            <button className="tp-btn tp-deal" disabled>Dealing...</button>
          )}
          {state === 'playing' && (
            <>
              <button className="tp-btn tp-call" onClick={showdown} disabled={loading}>✅ SHOW / CALL</button>
              <button className="tp-btn tp-fold" onClick={fold} disabled={loading}>🏳️ FOLD (lose {Math.floor(stake / 2)} pts)</button>
            </>
          )}
        </div>
      </div>
